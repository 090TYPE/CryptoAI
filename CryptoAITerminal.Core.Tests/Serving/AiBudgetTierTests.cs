...
///
/// Сейчас предел ДЕЙСТВУЕТ, а различие по тарифам — нет: тестовый период это «один предел на
/// всех», а не «предела нет». Первым здесь проверяется именно эта форма, потому что однажды её
/// перепутали — предел сняли целиком, и вместе с потолком ответа, поднятым в расчёте на этот
/// предел, платный ключ сервера остался без единого ограничения в токенах.
/// </summary>

...
public sealed class GridBotRunner
{
    ...
    public async Task PollAsync(GridConfig cfg, CancellationToken ct)
    {
        ...
        // Здесь намеренно НЕТ проверки «пустой снимок недостоверен», которая есть у настольного
        // GridBot, и намеренно нет двойного подтверждения: пропавший ордер сразу считается
        // исполненным. Это верно ровно потому, что пустой список тут значит буквально «живых
        // ордеров нет»:
        //   • бумажный шлюз детерминирован и честно отдаёт пустоту, когда цена пересекла все
        //     уровни разом — такую защиту пришлось бы обходить;
        //   • биржевые шлюзы на неуспехе запроса БРОСАЮТ, а не возвращают []; исключение уходит
        //     наверх в per-bot catch BotExecutorService, и тик просто пропускается.
        // Если когда-нибудь появится шлюз, отдающий [] вместо ошибки, ломаться будет здесь и
        // сразу всей сеткой: каждый опрос пометит все ордера исполненными и выставит встречные.

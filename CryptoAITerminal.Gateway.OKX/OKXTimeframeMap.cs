...
/// <summary>
/// Terminal timeframe token → OKX <see cref="KlineInterval"/>. The token list itself lives in
/// <see cref="TimeframeParser"/>; only the SDK mapping is local, and a token OKX has no interval
...
{
    /// <exception cref="ArgumentException">Unknown token, or one OKX has no interval for (8H).</exception>
    public static KlineInterval Parse(string? timeframe)
    {
        ...
        return (int)TimeframeParser.Parse(timeframe).TotalMinutes switch
        {
            ...
            _     => throw new ArgumentException(
                         $"OKX has no kline interval for timeframe '{timeframe}'.", nameof(timeframe)),
        };
    }
}

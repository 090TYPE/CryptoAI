...
///
/// Предел действует всегда; на тестовый период выключено РАЗЛИЧИЕ ПО ТАРИФАМ. Прежние числа
/// (35k/70k/105k в сутки) выбирались до того, как появились агент и панели с длинным контекстом, и
/// съедались за минуты, — поэтому пока все лицензии делят один общий предел, заведомо недостижимый
/// для честной работы. Полное снятие (<c>Enforced = false</c>) осталось аварийным рычагом: оно
/// оставляет платный ключ сервера вообще без потолка в токенах, и однажды так и вышло.
/// Считать расход при этом никто не перестаёт — <c>ai_usage</c> пишется по-прежнему, и именно из
/// него берутся цифры для будущего прайса.
/// </summary>

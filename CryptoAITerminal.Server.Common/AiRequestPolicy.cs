    ...
    long DailyTokenCapPerLicense)
{
    ...
    public static AiPolicyOptions FromConfig(Func<string, string?> cfg)
    {
        ...
        return new AiPolicyOptions(
            ...
            // Ровно по самому длинному законному запросу клиента: перевод интерфейса просит 6000,
            // панель сигналов — 5240, агентский виток — 4096. Зажим молчаливый, поэтому потолок
            // ниже реального потребления не экономит, а портит ответы; тест
            // AiFeatureCatalogueTests держит это равенство с каталогом функций.
            //
            // Было 8192 с обоснованием «настоящий ограничитель трат — суточная квота лицензии, а не
            // длина одного ответа». Потом квоту сняли на тестовый период, и обоснование исчезло
            // вместе с ней: запас в 2000 токенов сверх любого законного запроса стал просто
            // множителем к тому, что может выжать разогнавшийся клиент. Квота вернулась
            // предохранителем, но держать оба послабления разом незачем.
            MaxTokensCap: Int(cfg("AI_MAX_TOKENS_CAP"), 6000),

...
public static class SettingKeys
{
    ...
    /// <summary>
    /// Предел для лицензии, тариф которой не применяется или не опознан. Пока идёт тестовый период
    /// (<see cref="PlanTiersEnforced"/> выключен) это единственный действующий предел, один на всех.
    ///
    /// Щедрый намеренно и в обе стороны: незнакомый тариф означает, что план завели МЫ и забыли
    /// настроить, и обнаруживать это не должен платящий клиент.
    /// </summary>
    ...
    ///
    /// Сейчас НЕ ПРИМЕНЯЮТСЯ: см. <see cref="PlanTiersEnforced"/>. Числа выбирались до того, как
    /// появились агент и панели с длинным контекстом, и на живом использовании кончались за минуты.
    /// Оставлены как форма, а не как решение — при включении тарифов их надо пересчитать по
    ...
    /// <summary>
    /// Проверять ли суточный предел токенов вообще. По умолчанию ДА, и выключать это не следует.
    ///
    /// Было выключено на тестовый период — и это оказалось ошибкой в паре с другим изменением:
    /// потолок одного ответа подняли 2048 → 8192 с обоснованием «настоящий ограничитель трат —
    /// суточная квота, а не длина одного ответа», а квоту после этого сняли. Расцепив их, сервер
    /// остался без единого потолка в токенах: держателя валидной лицензии удерживал только
    /// рейт-лимит в 120 запросов в минуту, а это порядка полутора тысяч долларов в час на общий
    /// вендорский ключ.
    ///
    /// Правильная форма тестового периода — не «предела нет», а «предел общий и заведомо
    /// недостижимый для честной работы»: см. <see cref="PlanTiersEnforced"/>.
    /// </summary>
    ...
    public const bool DefaultAiBudgetEnforced = true;

    /// <summary>
    /// Применять ли РАЗНЫЕ пределы по тарифам. По умолчанию нет: идёт тестовый период, цена вызова
    /// ещё не измерена, и разделять клиентов по числам, выбранным до появления агента, значит
    /// отключать функции у тех, кто как раз и должен их проверить.
    ///
    /// Пока выключено, все лицензии получают один общий предел
    /// (<see cref="PlanDefaultDailyTokens"/>) — предохранитель от разгона, а не тариф. Учёт при
    /// этом полный: <c>AiBudget.Charge</c> и строка в <c>ai_usage</c> пишутся на каждый вызов,
    /// и именно из них потом возьмутся тарифные числа.
    /// </summary>
    public const string PlanTiersEnforced = "plan.tiers.enforced";

    /// <summary>Значение <see cref="PlanTiersEnforced"/> по умолчанию.</summary>
    public const bool DefaultPlanTiersEnforced = false;

    /// <summary>
    /// Общий суточный предел на лицензию, пока тарифы не применяются.
    ///
    /// Два миллиона — это примерно шестьдесят раз по прежней квоте Pro. Честная работа сюда не
    /// упирается даже близко: самая дорогая панель просит 6000 токенов на ответ, а агентский виток
    /// — 4096, то есть до потолка нужно порядка трёхсот полновесных вызовов в сутки подряд. Зато
    /// разгон в 120 запросов в минуту упирается в него за четверть часа, и дальше лицензия стоит
    /// ноль вместо тысяч долларов.
    ///
    /// Это предохранитель, а не прайс. Тарифные числа появятся из <c>ai_usage</c>.
    /// </summary>
    public const long DefaultTestPeriodDailyTokens = 2_000_000;

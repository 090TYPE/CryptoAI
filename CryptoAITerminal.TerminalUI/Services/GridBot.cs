...
public sealed class GridBot : IDisposable
{
    ...
    private async Task PollFillsAsync()
    {
        ...
        try
        {
            ...
            // Пустой снимок при нескольких живых лимитках — это почти всегда сбой запроса, а не
            // одновременный филл всей сетки. Сами шлюзы теперь на неуспехе бросают, и тик
            // пропускается ниже по catch; проверка остаётся защитой от шлюза, который всё-таки
            // ответит пустотой (в том числе от заглушки интерфейса). Порог именно >1: сетка из
            // одного уровня, исполнившись, законно даёт пустой снимок, и его надо увидеть.

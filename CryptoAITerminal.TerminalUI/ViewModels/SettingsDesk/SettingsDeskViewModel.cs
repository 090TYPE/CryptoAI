...
public sealed class SettingsDeskViewModel : ReactiveObject
{
    ...
    {
        // Расход показывается всегда: это единственное место, где пользователь видит, во что
    ...
    public string SubRemaining => _sub switch
    {
        ...
        // Предохранитель и тариф — разные вещи, и подписаны по-разному: обещать «осталось столько-то
        // по вашему тарифу» там, где тарифы ещё не применяются, значит продавать то, чего нет.
        { Tiered: false, Cap: > 0 } s =>
            $"тестовый период · общий предел {s.Cap / 1000.0:0}k на страховку от разгона, тариф пока не применяется · " +
            $"счётчик обнулится в {s.ResetsUtc.ToLocalTime():HH:mm}",
    ...
    public string SubStatusText => _sub switch
    {
        ...
        { Tiered: false } => "Тестовый период: тарифные квоты не применяются, все лицензии делят один общий предел — " +
                             "страховку от разгона, до которой обычная работа не доходит. Ключ и модель — на стороне " +
                             "сервиса, расход считается, чтобы посчитать по нему тариф.",

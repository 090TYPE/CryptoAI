...
> | View, привязанных к `MainWindowViewModel` | 40 | **4** |
> | View с компилируемыми биндингами | 0 | **34** |
...
>
> **`MarketsView`** — держится за пару `Markets` + `SelectedMarket`. Сеттер `SelectedMarket`
> это хаб смены символа всего приложения: обновление стакана, лента, котировки площадок,
> свечи, фьючерсный аккаунт. Владение уже переехало в `MarketFeedOwnerViewModel`, но сам
> экран остаётся на оболочке, потому что читает её же.
>
...
> на ETHUSDT — это покупка, которая исполнится мгновенно). Все исправлены.

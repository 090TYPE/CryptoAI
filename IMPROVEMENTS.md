...
> ## Статус: закрыто, кроме двух пунктов
>
...
> | View, привязанных к `MainWindowViewModel` | 40 | **7** |
> | View с компилируемыми биндингами | 0 | **33** |
> | Пустых `catch` без объяснения | 21 | **0** (13 осознанных, у каждого комментарий) |
> | Биндингов цвета мимо `StringToBrush` | 247 | **37** |
> | Использований DI-контейнера | 0 | композиционный корень в `Program.cs` |
>
> **Осталось два пункта, и оба — не «не успел», а «нельзя сделать скриптом».**
>
> **`TradingDeskView`** — 347 корневых путей привязки, 237 уже через под-вьюмодели,
> 110 напрямую в оболочку. Класс-композиция поверх 110 членов — это та же связанность
> с лишним файлом перед ней. Нужно извлечение по группам: тикет ордера, стакан, позиция,
> график, DEX-панель.
>
> **`MarketsView` и четыре виджета** (`PriceStats`, `OrderBook`, `TrackedCoins`, `PriceChart`) —
> держатся за пару `Markets` + `SelectedMarket`. Сеттер `SelectedMarket` это хаб смены символа
> всего приложения: обновление стакана, лента, котировки площадок, свечи, фьючерсный аккаунт.
> Ту же пару читают ещё три View и два сервиса. Это выделение **сервиса рыночного фида**,
> а не развязка экрана, и по одному View оно не делается.
>
...
> не на тот объект. Оба исправлены.

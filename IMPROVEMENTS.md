...
> ## Статус: все 109 пунктов закрыты
>
...
> | Публичных членов у оболочки | — | 878 → **777** |
...
>
> **Тикет ордера вынесен — последний пункт закрыт.** `OrderTicketViewModel` владеет всем
> изменяемым состоянием исполнения: сторона, тип, режим рынка, профиль, лимит, TP, SL,
> количество, проскальзывание, плечо, режим маржи — плюс производные от них (нотионал,
> комиссия, сетевой сбор, итоговая стоимость, текст и подсказка кнопки, гард
> `CanPlacePrimaryOrder`) и пять команд выбора. Сама отправка ордера осталась в оболочке:
> `PlacePrimaryOrderAsync`, девять `GetCex*BlockedReason`, пресеты и риск-проверки читают
> тикет через `OrderTicketVM.X`, а команда возвращается тикету через `AttachCommands` —
> тем же приёмом, каким `CexRightRailViewModel` получает CLOSE / REVERSE.
> Оркестрация из сеттеров разорвана событиями (`TicketStateChanged`, `MarketModeChanged`,
> `LeverageChanged`, `MarginModeChanged`, `TradingProfileChanged`, `OrderSideSelected`);
> обработчики в оболочке — дословные хвосты прежних сеттеров.
> `Services/AppActions/MainWindowAppActionContext.cs` и мутаторы `*FromAgent` переведены туда же.
>
> Проверялось не только сборкой: до рефакторинга снят поведенческий слепок тикета
> (профиль → проскальзывание и TP/SL, MARKET/LIMIT, SPOT/FUTURES, BUY/SELL, формула
> «нотионал + комиссия 0,10% + газ», текст гарда), после — сверен по пунктам на живом
> приложении. Совпало всё, включая живое обновление бейджа плеча в правой панели при
> движении слайдера — это цепочка через два разных вынесенных вьюмодела.
>
> В `TradingDeskView` в оболочку теперь смотрят **19** членов вместо 43 (телеметрия
> фьючерсов, лейблы гарда и сессии, спред, площадка). Это уже в тех рамках, где
> `TradingScreenViewModel` был бы композицией, а не прокси, — но это следующая задача,
> а не пункт аудита.
>

...
- **Суточный предел действует, тарифы — нет.** Два разных переключателя в `/admin` → «Суточные
  квоты»: `ai.budget.enforced` (по умолчанию `true`) — предохранитель от разгона, выключать его не
  следует; `plan.tiers.enforced` (по умолчанию `false`) — пока выключен, все лицензии делят один
  общий предел `plan.default.daily_tokens` (2 000 000 токенов в сутки). Расход считается всегда и
  пишется в `ai_usage` — именно из него потом берутся числа для тарифов. Перед включением тарифов
  эти числа надо пересчитать: зашитые в коде взяты до появления агента и панелей с длинным
  контекстом.
